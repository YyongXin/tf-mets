...
class Platform {
 public:
  ...
  CreateSwapCudaGPUs(const std::string& blk_sel_mode, const std::string& blk_sched_mode, const int& mem_limit, const size_t num_devices, size_t init_size = 0);
  static const std::vector<std::shared_ptr<Device>>
  CreateSwapCudaGPUsOn(const std::string& blk_sel_mode, const std::string& blk_sched_mode, const int& mem_limit, const std::vector<int> &devices, size_t init_size = 0);
#endif // USE_CUDA
